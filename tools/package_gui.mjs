...
for(const required of ['MFAAvalonia.exe','runtimes/node/node.exe','runtimes/maa/node_modules/@maaxyz/maa-node/dist/index-server.js','runtimes/dotnet/dotnet.exe','assets/resource/model/ocr/rec.onnx']){
...
for(const name of ['assets','tools','docs','libs','plugins','MaaAgentBinary','README.md','LICENSE','package.json','package-lock.json','run_gui.cmd','run_daily.cmd','update_runtime.cmd','libloader.dll'])if(fs.existsSync(path.join(root,name)))copy(name)
